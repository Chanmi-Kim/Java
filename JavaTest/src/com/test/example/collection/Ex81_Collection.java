...
public class Ex81_Collection {
	...
	private static void m2() {
		...
		//내가 원하는 기능???
		//1. 객체.멤버 -> 해당 데이터와 관련된 기능
		//2. 클래스명.멤버 -> 타입과 관련된 공통(범용) 기능
		//3. 다른 클래스 -> 유틸 클래스 제공
		
		//덤프(dump)
		System.out.println(names); //컬렉션은 toString() 오버라이드 되어 있다.
		System.out.println(Arrays.toString(nums)); //**************
		...
		Arrays.fill(nums, 5, 6, 50);//범위 지정
		
		System.out.println(Arrays.toString(nums));
		
		
		int[] copy = nums.clone();//깊은 복사
		...
		copy = Arrays.copyOf(nums, 10);
		System.out.println(Arrays.toString(copy));
		
		copy = Arrays.copyOfRange(nums, 2, 4);//세밀하게 범위 지정 가능
		System.out.println(Arrays.toString(copy));
